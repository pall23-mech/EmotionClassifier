...
def get_conv_model(input_shape):
    ...
    model.add(BatchNormalization())
    model.add(MaxPooling2D((2, 2)))  # Pooling after two Conv layers
    ...
    model.add(BatchNormalization())
    # Skip pooling here, retain more features

    # Third block
    model.add(Conv2D(512, (3, 3), activation='relu', padding='same', kernel_regularizer=l2(0.0005)))
    model.add(BatchNormalization())
    model.add(MaxPooling2D((2, 2)))  # Pooling here to reduce dimensionality
    
    # Fourth block
    model.add(Conv2D(1024, (3, 3), activation='relu', padding='same', kernel_regularizer=l2(0.0005)))
    model.add(BatchNormalization())
    # Add pooling here only if necessary to match the target size for dense layers
