...
from tensorflow.keras.layers import Dense, Conv1D, MaxPooling1D, Dropout, BatchNormalization, LSTM, Activation
...
def stretch(data, rate=0.70):
    if isinstance(data, np.ndarray):
        ...
        raise ValueError("Expected numpy array for 'data' in stretch function.")
...
def pitch(data, sampling_rate, pitch_factor=0.8):
    if isinstance(data, np.ndarray):
        ...
        raise ValueError("Expected numpy array for 'data' in pitch function.")

def higher_speed(data, speed_factor=1.25):
    if isinstance(data, np.ndarray):
        return librosa.effects.time_stretch(data, rate=speed_factor)
    else:
        raise ValueError("Expected numpy array for 'data' in higher_speed function.")

def lower_speed(data, speed_factor=0.75):
    if isinstance(data, np.ndarray):
        return librosa.effects.time_stretch(data, rate=speed_factor)
    else:
        raise ValueError("Expected numpy array for 'data' in lower_speed function.")
...
from tensorflow.keras.layers import Dense, Permute, Multiply, Lambda
...
def self_attention(input_tensor):
    ...
    attention_output = Multiply()([input_tensor, attention])
...
# Modified Model Creation (CNN-LSTM with Self-Attention)
import tensorflow as tf
